...
describe('App', () => {
  ...
  it("interacts with settings modal thoroughly", async () => {
    ...
    await waitFor(() => expect(screen.getByText("AI Model Settings")).toBeInTheDocument());
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining("/fetch-models"), expect.any(Object));
    });
    ...
    await waitFor(() => {
      expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining("/api/test-connection"), expect.any(Object));
    });

    const apiKeyInput = screen.getByPlaceholderText("google_api from env if empty");
  ...
  it("handles dashboard empty states and refresh", async () => {
    ...
    fireEvent.click(screen.getByText("Trading Bot"));
    await waitFor(() => {
        expect(screen.getByText("No approved stocks yet. Add one above.")).toBeInTheDocument();
    });
