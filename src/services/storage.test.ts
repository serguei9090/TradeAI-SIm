...
describe("Storage Service", () => {
	...
	it("getTradeHistory", async () => {
		...
		const _res = await storage.getTradeHistory();
		expect(global.fetch).toHaveBeenCalledWith("/api/db/trade-history");
	});

	it("getApprovedStocks", async () => {
		(global.fetch as any).mockResolvedValueOnce({
			json: () => Promise.resolve([{ symbol: "TSLA" }]),
		});
		const _res = await storage.getApprovedStocks();
	...
	it("approveStock", async () => {
		...
		const _res = await storage.approveStock("TSLA");
		expect(global.fetch).toHaveBeenCalledWith(
			"/api/db/approved-stocks",
	...
	it("removeApprovedStock", async () => {
		...
		const _res = await storage.removeApprovedStock("TSLA");
		expect(global.fetch).toHaveBeenCalledWith(
			"/api/db/approved-stocks/TSLA",
	...
	it("getSettings", async () => {
		(global.fetch as any).mockResolvedValueOnce({
			json: () => Promise.resolve({ tradeSize: 5 }),
		});
		const _res = await storage.getSettings();
	...
	it("updateSettings", async () => {
		...
		const _res = await storage.updateSettings({ tradeSize: 10 });
		expect(global.fetch).toHaveBeenCalledWith(
			"/api/db/settings",
			expect.objectContaining({ method: "POST" }),
		);
	...
	it("addFunds", async () => {
		...
		const _res = await storage.addFunds(1000);
		expect(global.fetch).toHaveBeenCalledWith(
			"/api/db/portfolio/add-funds",
			expect.objectContaining({ method: "POST" }),
		);
	});
	...
	it("getAiLogs", async () => {
		...
		const _res = await storage.getAiLogs();
