	...
	try {
		...
		if (apiKey) {
			...
			const newsRes = await fetch(
				`https://finnhub.io/api/v1/company-news?symbol=${symbol}&from=${lastWeek}&to=${today}&token=${apiKey}`,
			);
...
Then, on a new line, provide a short 1-sentence reasoning.`;
		...
		const reasoning =
			lines.slice(1).join(" ").trim() || "No reasoning provided";
		...
		db.run("INSERT INTO ai_logs (symbol, log) VALUES (?, ?)", [
			symbol,
			logEntry,
		]);
		...
		if (decision === "BUY" || decision === "SELL") {
			...
			const stopLoss = (price) =>
				price * (1 - (settings.stopLossPct || 10) / 100);
			const takeProfit = (price) =>
				price * (1 + (settings.takeProfitPct || 10) / 100);
...
export const startEngine = () => {
	setInterval(() => {
		...
		db.all("SELECT symbol FROM approved_stocks", (err, rows: any[]) => {
			if (err)
				return console.error("Error fetching approved stocks for engine");
			rows.forEach((row) => {
