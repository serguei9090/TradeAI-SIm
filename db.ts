	...
	if (err) {
        ...
        totalValue REAL NOT NULL
			...
			db.get("SELECT count(*) as count FROM portfolios", (err, row: any) => {
				if (!err && row.count === 0) {
					db.run(
						"INSERT INTO portfolios (id, balance, totalValue) VALUES ('default', 10000.0, 10000.0)",
					);
				}
			});
        ...
        maxPositions INTEGER
			...
			db.all("PRAGMA table_info(settings)", (err, columns: any[]) => {
				if (!err && columns) {
					const hasModelProvider = columns.some(
						(col) => col.name === "modelProvider",
					);
					if (!hasModelProvider) {
						db.run(
							"ALTER TABLE settings ADD COLUMN modelProvider TEXT DEFAULT 'custom'",
						);
					}
				}
			});
