...
describe("Trading Engine", () => {
	...
	it("evaluateStock ignores execution if not running", async () => {
		(db.all as any).mockImplementation((_query: any, cb: any) =>
			cb(null, [{ symbol: "AAPL" }]),
		);
		startEngine();
		vi.advanceTimersByTime(10000);
	...
	it("evaluateStock completes full cycle (custom provider, SELL)", async () => {
		...
		(db.get as any).mockImplementationOnce((_query: any, cb: any) =>
			cb(null, { modelProvider: "custom", tradeSize: 1 }),
		);
		// second db.get is within executeTrade
		(db.get as any).mockImplementationOnce(
			(_query: any, params: any, cb: any) => {
		...
		(db.get as any).mockImplementationOnce(
			(_query: any, _params: any, cb: any) => cb(null, { shares: 10 }),
		);
		...
		global.fetch = vi
			...
			.mockResolvedValueOnce({
				...
				json: vi
					.fn()
					.mockResolvedValue([{ headline: "News 1", summary: "Sum 1" }]),
			})
			.mockResolvedValueOnce({
				// AI chat
	...
	it("evaluateStock gemini no API key", async () => {
		...
		await vi.waitFor(() => {
			...
			expect(db.run).not.toHaveBeenCalledWith(
				expect.stringContaining("INSERT INTO ai_logs"),
				expect.anything(),
			);
		});
	});
