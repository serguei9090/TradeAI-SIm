...
describe('Backend Routes', () => {
  ...
  describe('Portfolio', () => {
    it('GET /api/portfolio should return portfolio', async () => {
      (db.get as any).mockImplementationOnce((query: any, cb: any) => cb(null, { balance: 5000 }));
    ...
    it('GET /api/portfolio should handle errors', async () => {
      (db.get as any).mockImplementationOnce((query: any, cb: any) => cb(new Error("DB Error")));
      const res = await request(app).get('/api/portfolio');
  ...
  describe('Add Funds', () => {
    it('POST /api/portfolio/add-funds should update balance and return success', async () => {
      (db.run as any).mockImplementationOnce((query: any, params: any, cb: any) => cb.call(null, null));
    ...
    it('POST /api/portfolio/add-funds should return 400 for invalid amounts', async () => {
      const res = await request(app).post('/api/portfolio/add-funds').send({ amount: -500 });
      expect(res.status).toBe(400);
    });

    it('POST /api/portfolio/add-funds should handle errors', async () => {
      (db.run as any).mockImplementationOnce((query: any, params: any, cb: any) => cb.call(null, new Error("DB Error")));
      const res = await request(app).post('/api/portfolio/add-funds').send({ amount: 5000 });
      expect(res.status).toBe(500);
    });
  });
  ...
  describe('Positions', () => {
    it('GET /api/positions should return positions', async () => {
      (db.all as any).mockImplementationOnce((query: any, cb: any) => cb(null, [{ id: 1, symbol: 'AAPL' }]));
    ...
    it('GET /api/positions should handle errors', async () => {
      (db.all as any).mockImplementationOnce((query: any, cb: any) => cb(new Error("DB Error")));
      const res = await request(app).get('/api/positions');
      expect(res.status).toBe(500);
    });
  });
  ...
  describe('Trade History', () => {
    it('GET /api/trade-history should return history', async () => {
      (db.all as any).mockImplementationOnce((query: any, cb: any) => cb(null, [{ id: 1, symbol: 'AAPL', type: 'BUY' }]));
  ...
  describe('Approved Stocks', () => {
    it('GET /api/approved-stocks should return stocks', async () => {
      (db.all as any).mockImplementationOnce((query: any, cb: any) => cb(null, [{ symbol: 'AAPL' }]));
    ...
    it('POST /api/approved-stocks should add stock', async () => {
      (db.run as any).mockImplementationOnce((query: any, params: any, cb: any) => cb.call(null, null));
      const res = await request(app).post('/api/approved-stocks').send({ symbol: 'TSLA' });
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
  ...
  describe('Trade', () => {
    it('POST /api/trade BUY should succeed with sufficient funds', async () => {
      (db.get as any).mockImplementationOnce((query: any, cb: any) => cb(null, { balance: 10000 }));
      (db.run as any).mockImplementation((query: any, params: any, cb: any) => { if(cb) cb.call(null, null); });

      const res = await request(app).post('/api/trade').send({ symbol: 'AAPL', type: 'BUY', shares: 1, price: 150 });
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });

    it('POST /api/trade BUY should fail with insufficient funds', async () => {
      (db.get as any).mockImplementationOnce((query: any, cb: any) => cb(null, { balance: 100 }));
    ...
    it('POST /api/trade SELL should succeed', async () => {
      ...
      const res = await request(app).post('/api/trade').send({ symbol: 'AAPL', type: 'SELL', shares: 1, price: 150 });
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });

    it('POST /api/trade INVALID should fail', async () => {
      (db.get as any).mockImplementationOnce((query: any, cb: any) => cb(null, { balance: 1000 }));

      const res = await request(app).post('/api/trade').send({ symbol: 'AAPL', type: 'INVALID', shares: 1, price: 150 });
      expect(res.status).toBe(400);
  ...
  describe('Settings', () => {
    it('GET /api/settings should return settings', async () => {
      (db.get as any).mockImplementationOnce((query: any, cb: any) => cb(null, { customApiUrl: 'http://test', tradeSize: 2 }));
    ...
    it('POST /api/settings should update settings', async () => {
      (db.run as any).mockImplementationOnce((query: any, params: any, cb: any) => cb.call(null, null));
      const res = await request(app).post('/api/settings').send({ tradeSize: 3 });
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });
  ...
  describe('AI Chat', () => {
    ...
    it('POST /api/ai-chat should return a reply for custom provider', async () => {
      (db.get as any).mockImplementationOnce((query: any, cb: any) => cb(null, { modelProvider: 'custom' }));
